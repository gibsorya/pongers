defmodule PongersWeb.MatchController do
  use PongersWeb, :controller

  alias Pongers.Events
  alias Pongers.Events.Match

  action_fallback PongersWeb.FallbackController

  def index(conn, _params) do
    matches = Events.list_matches()
    render(conn, :index, matches: matches)
  end

  def create(conn, %{"match" => match_params}) do
    with {:ok, %Match{} = match} <- Events.create_match(match_params) do
      conn
      |> put_status(:created)
      |> put_resp_header("location", ~p"/api/matches/#{match}")
      |> render(:show, match: match)
    end
  end

  def show(conn, %{"id" => id}) do
    match = Events.get_match!(id)
    render(conn, :show, match: match)
  end

  def update(conn, %{"id" => id, "match" => match_params}) do
    match = Events.get_match!(id)

    with {:ok, %Match{} = match} <- Events.update_match(match, match_params) do
      render(conn, :show, match: match)
    end
  end

  def delete(conn, %{"id" => id}) do
    match = Events.get_match!(id)

    with {:ok, %Match{}} <- Events.delete_match(match) do
      send_resp(conn, :no_content, "")
    end
  end
end
